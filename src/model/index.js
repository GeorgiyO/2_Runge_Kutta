...
import {rungeKutta, rungeKuttaSecondOrder} from "./RungeKutta";
...
const props = {
    inputPanel: {
        ...
        onRk4Change: (e) => {
            ...
            ip.state[mode].rk4 = e.target.checked;
            ip.updateState();
            if (ip.state.autoUpdate) updateData();
        },
        updateData,
...
function updateDataFO() {
    let {x0, y0, h, iterations, rk4, foo, realFoo} = props.inputPanel.state.firstOrder;
    ...
    updateDataAndGraphState(rungeKutta(x0, y0, h, iterations, rk4, foo), realFoo);
}

function updateDataSO() {
    let {x0, y0, dy0, h, iterations, rk4, foo, realFoo} = props.inputPanel.state.secondOrder;
    ...
    updateDataAndGraphState(rungeKuttaSecondOrder(x0, y0, dy0, h, iterations, rk4, foo), realFoo);
}
